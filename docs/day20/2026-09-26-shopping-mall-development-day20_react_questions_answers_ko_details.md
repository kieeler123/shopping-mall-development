# Day 20 React 기초 통합 --- 문제 & 정답 대용량 문제집

> 각 정답은 **정답 보기**를 클릭하면 펼쳐집니다. 범위: Day 18\~20 /
> Next.js + TypeScript\
> 구성: 개념 → 코드 읽기 → 빈칸 → 오류 수정 → 직접 작성 → 통합 설계\
> 권장법: 먼저 문제만 풀고, 각 섹션 아래 정답을 나중에 확인한다.

## Part 1. JavaScript / Web 기초

### 문제

1.  배열과 객체의 차이를 설명하라.
2.  `...products`가 배열 안에서 하는 역할은?
3.  다음 결과의 배열 순서는?
...
4.  `map`은 원본 배열을 직접 수정하는가?
5.  `filter`의 callback이 `true`를 반환한 요소는 어떻게 되는가?
6.  `"50000"`을 숫자 `50000`으로 변환하라.
...
**팁:** React 문제처럼 보여도 `map`, `filter`, `spread`, `Number()`에서
막혔다면 먼저 JavaScript 문제로 분리한다.
...
17. 이전 count를 기준으로 1 증가시키는 코드를 작성하라.
18. 왜 `count = count + 1` 대신 setter를 사용하는가?
19. 함수형 업데이트가 특히 유용한 상황은?
20. 다음 빈칸을 채워라.
...
**팁:** `setState(prev => next)`를 하나의 기본 뼈대로 익혀두면 배열
추가·삭제에도 그대로 적용할 수 있다.

---

## Part 3. 불변성 / Object / Array / Spread

### 문제

23. React State에서 불변 업데이트란?
...
**팁:** 객체 spread에서는 순서가 중요하다. `{ ...old, name: newName }`은
기존 값을 복사한 뒤 `name`만 덮어쓴다.

---

## Part 4. Product 타입 / TypeScript

### 문제

31. 다음 타입을 작성하라: id는 number, name은 string, price는 number인
    `Product`.
32. `Product`와 `Product[]`의 차이는?
33. 다음 중 올바른 값은?
...
**팁:** TypeScript 오류를 만나면 먼저
`지금 필요한 것은 객체인가, 배열인가, 함수인가?`를 분류한다.

---

## Part 5. Controlled Component / Event

### 문제

41. Controlled Component란?
...
모두 초기화하는 코드를 작성하라. 50. `onChange`와 `onSubmit`의 역할
차이는?
...
**팁:** Controlled input은 `value + onChange + State` 세 요소를 한
세트로 본다.
...
**팁:** `propName={value}`를 볼 때 항상 `이름`과 `실제 값`을 분리해서
읽는다.
...
**팁:** `Product[] → map → Product → ProductItem`을 하나의 고정 흐름처럼
익힌다.
...
87. 자식 Event → 함수 Props → 부모 handler → setter → 부모 State 변경 →
    Props → 재렌더링.

</details>

**팁:** 함수 Props도 결국 값 전달이다. 차이는 자식이 그 함수를 나중에
실행한다는 점이다.
...
93. `filter`, `!==`
94. 클릭 → `product.id` → 함수 Props → 부모 `handleDelete` → `filter` →
    새 배열 → `setProducts` → State 변경 → 재렌더링 → UI.
...
**팁:** 오류 수정에서는 코드를 통째로 다시 쓰기보다 `타입`, `State`,
...
**팁:** 이 파트는 정답을 외우지 말고, 정답을 가린 상태에서 여러 번 직접
타이핑한다.
...
126. ProductItem 클릭 → `handleDelete(product.id)` → 함수 Props → App
     handler → filter → setProducts → 재렌더링.
...
**팁:** 긴 문장을 외우지 말고 단계별 화살표를 직접 그려본다.
...
136번에 삭제 기능을 추가하라. 삭제 함수는 App에 두고 함수 Props로
ProductItem까지 전달하라.
...
`ProductForm`이 반드시 받아야 할 값 Props와 함수 Props를 현재 구조
기준으로 나열하라.
...
136. App이 State/handler를 관리하고 Form/List/Item으로 필요한 Props를
     전달한다.
137. `handleDelete(id)` → `filter` → `setProducts`; 함수 Props를 List를
     거쳐 Item까지 전달한다.
...
**팁:** 133\~137은 정답을 읽는 것보다 빈 파일에서 직접 완성하는 것이
시험의 핵심이다.
...
**팁:** 143\~162를 코드 없이 답하고, 막힌 항목만 해당 파트로 돌아가
복습한다.

# Day 17 --- STEP 15. createdOrder --- 最大限詳しく説明した版

> **Day 17の大テーマ:** Day 15 HTTP/API + Day 16
> 非同期JavaScriptを`fetch()`とReact Stateへ接続する\
> **STEP 15の目標:** Client
> inputではなくServerが実際に作成して返したResourceをStateへ追加する理由を理解する。
...
``` text
input → Serverが最終値生成 → createdOrder → Stateへ追加
```
...
コードなしで説明できればSTEP 15の中心を理解できている。

# Day 17 --- STEP 18. useOrders HTTP 리팩터링 --- 최대한 풀어쓴 버전

> **Day 17 큰 주제:** Day 15 HTTP/API + Day 16 비동기 JavaScript를
> `fetch()`와 React State에 연결하기\
> **STEP 18 핵심 목표:** Component의 외부 인터페이스는 유지하면서
> `useOrders` 내부 Data Source를 localStorage에서 HTTP로 바꾸는 책임
> 분리를 이해한다.
...
``` text
page는 무엇을 할지 표현하고 Hook은 데이터 조회/변경 방법을 담당
```
...
Component의 외부 인터페이스는 유지하면서 `useOrders` 내부 Data Source를
localStorage에서 HTTP로 바꾸는 책임 분리를 이해한다.
...
> Component의 외부 인터페이스는 유지하면서 `useOrders` 내부 Data
> Source를 localStorage에서 HTTP로 바꾸는 책임 분리를 이해한다.

이 문장을 코드 없이 설명할 수 있다면 STEP 18의 핵심을 이해한 것이다.

# Day 17 --- STEP 13. 특정 주문 GET --- 최대한 풀어쓴 버전

> **Day 17 큰 주제:** Day 15 HTTP/API + Day 16 비동기 JavaScript를
> `fetch()`와 React State에 연결하기\
> **STEP 13 핵심 목표:** `/orders` Collection과 `/orders/:id` 개별
> Resource의 차이 및 200/404 응답을 이해한다.
...
`/orders` Collection과 `/orders/:id` 개별 Resource의 차이 및 200/404
응답을 이해한다.
...
이 문장을 코드 없이 설명할 수 있다면 STEP 13의 핵심을 이해한 것이다.

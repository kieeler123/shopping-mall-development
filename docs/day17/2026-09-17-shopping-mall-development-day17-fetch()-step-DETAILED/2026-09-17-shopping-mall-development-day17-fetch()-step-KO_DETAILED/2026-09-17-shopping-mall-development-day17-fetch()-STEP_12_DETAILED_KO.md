# Day 17 --- STEP 12. Error State --- 최대한 풀어쓴 버전

> **Day 17 큰 주제:** Day 15 HTTP/API + Day 16 비동기 JavaScript를
> `fetch()`와 React State에 연결하기\
> **STEP 12 핵심 목표:** HTTP/네트워크 실패를 React State에 저장하여
> 실패 UI로 연결하고 Loading/Error/Empty/Data 상태를 구분한다.
...
``` text
실패 → Error State → Re-render → Error UI
```
...
HTTP/네트워크 실패를 React State에 저장하여 실패 UI로 연결하고
Loading/Error/Empty/Data 상태를 구분한다.
...
이 문장을 코드 없이 설명할 수 있다면 STEP 12의 핵심을 이해한 것이다.

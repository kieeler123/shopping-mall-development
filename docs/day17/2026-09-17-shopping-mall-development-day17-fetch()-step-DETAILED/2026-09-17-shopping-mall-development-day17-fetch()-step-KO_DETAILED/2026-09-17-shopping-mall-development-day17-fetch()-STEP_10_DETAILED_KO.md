# Day 17 --- STEP 10. React 주문 목록 GET --- 최대한 풀어쓴 버전

> **Day 17 큰 주제:** Day 15 HTTP/API + Day 16 비동기 JavaScript를
> `fetch()`와 React State에 연결하기\
> **STEP 10 핵심 목표:** 서버에서 받은 `Order[]`를 `setOrders`로 State에
> 반영해 UI가 다시 렌더링되는 과정을 연결한다.
...
서버에서 받은 `Order[]`를 `setOrders`로 State에 반영해 UI가 다시
렌더링되는 과정을 연결한다.
...
이 문장을 코드 없이 설명할 수 있다면 STEP 10의 핵심을 이해한 것이다.

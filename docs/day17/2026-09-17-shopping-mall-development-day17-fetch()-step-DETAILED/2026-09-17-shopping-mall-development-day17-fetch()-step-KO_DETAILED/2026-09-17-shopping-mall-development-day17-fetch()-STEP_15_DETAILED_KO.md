# Day 17 --- STEP 15. createdOrder --- 최대한 풀어쓴 버전

> **Day 17 큰 주제:** Day 15 HTTP/API + Day 16 비동기 JavaScript를
> `fetch()`와 React State에 연결하기\
> **STEP 15 핵심 목표:** POST 후 Client 입력값이 아니라 서버가 실제
> 생성해 반환한 Resource를 State에 추가해야 하는 이유를 이해한다.
...
``` text
input → Server 생성(ID/시간 등) → createdOrder → State append
```
...
POST 후 Client 입력값이 아니라 서버가 실제 생성해 반환한 Resource를
State에 추가해야 하는 이유를 이해한다.
...
이 문장을 코드 없이 설명할 수 있다면 STEP 15의 핵심을 이해한 것이다.

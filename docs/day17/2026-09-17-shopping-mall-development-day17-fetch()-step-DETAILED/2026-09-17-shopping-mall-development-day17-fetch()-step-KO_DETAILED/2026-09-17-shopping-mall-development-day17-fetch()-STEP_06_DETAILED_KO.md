# Day 17 --- STEP 06. response.ok와 HTTP Error --- 최대한 풀어쓴 버전

> **Day 17 큰 주제:** Day 15 HTTP/API + Day 16 비동기 JavaScript를
> `fetch()`와 React State에 연결하기\
> **STEP 06 핵심 목표:** 404/500 같은 HTTP 오류와 fetch 자체의 네트워크
> 실패를 구분하고 `response.ok`를 확인하는 이유를 이해한다.
...
``` text
HTTP Response 도착 → response.ok 확인 → non-2xx이면 실패 흐름으로 전환
```
...
404/500 같은 HTTP 오류와 fetch 자체의 네트워크 실패를 구분하고
`response.ok`를 확인하는 이유를 이해한다.
...
이 문장을 코드 없이 설명할 수 있다면 STEP 06의 핵심을 이해한 것이다.

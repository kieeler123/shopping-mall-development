...
> **この STEP の目標:** 注文作成を通して POST、Request Body、Server
> validation、201 Created、非冪等性を理解する。
...
## 2. CRUD API で Create に POST を使う理由
...
## 5. newOrder はまだ確定 Resource ではない

`newOrder はまだ確定 Resource ではない`は単独の定義としてではなく、一つの
...
> **ヒント**
>
> `newOrder はまだ確定 Resource ではない`を読んだ後、注文の取得・作成・更新・削除のどれか一つを使って、自分の言葉で説明してみよう。

## 6. Client が送る価格を Server が信頼すべきでない理由
...
## 12. createdOrder と newOrder が異なり得る理由

`createdOrder と newOrder が異なり得る理由`は単独の定義としてではなく、一つの
end-to-end HTTP flow の中で理解する。
...
> **ヒント**
>
> `createdOrder と newOrder が異なり得る理由`を読んだ後、注文の取得・作成・更新・削除のどれか一つを使って、自分の言葉で説明してみよう。

## 13. POST が safe でない理由
...
1.  **POST と Create**は注文 Project で何を意味するか？
...
-   [ ] 注文作成を通して POST、Request Body、Server validation、201
    Created、非冪等性を理解する。

...
> **이번 STEP의 목표:** Client가 Request를 보내고 Server가 Response를
> 반환하는 구조를 이해하고 Server와 Database를 구분한다.
...
## 4. Request와 Response의 방향

`Request와 Response의 방향`는 이번 STEP의 핵심 개념을 주문 프로젝트의
...
> **팁**
>
> `Request와 Response의 방향`를 공부한 뒤 주문 조회·생성·수정·삭제 중
> 하나의 실제 상황으로 다시 설명해보자.

## 5. Server와 Database의 차이
...
## 9. Client validation과 Server validation
...
`전체 흐름과 오해`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에
...
> **팁**
>
> `전체 흐름과 오해`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제
> 상황으로 다시 설명해보자.
...
1.  `Client와 Server`을 다른 HTTP 구성요소 전체와 같은 개념으로 취급하지
    않는다.
...
1.  **Client와 Server**을 코드 없이 자신의 말로 설명해보자.
...
-   [ ] Client가 Request를 보내고 Server가 Response를 반환하는 구조를
    이해하고 Server와 Database를 구분한다.

...
> **이번 STEP의 목표:** Resource, API, Endpoint의 관계를 이해하고 API를
> 단순 URL이나 Server 자체와 동일시하지 않는다.
...
`Resource`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에
...
> **팁**
>
> `Resource`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제
> 상황으로 다시 설명해보자.

## 2. orders collection

`orders collection`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP
...
> **팁**
>
> `orders collection`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의
> 실제 상황으로 다시 설명해보자.

## 3. 개별 Resource

`개별 Resource`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에
배치하는 과정이다. 정의 하나를 외우는 것보다 이 개념이 **어느 계층에
있고 어떤 질문에 답하는지**를 이해해야 한다.
...
> **팁**
>
> `개별 Resource`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제
> 상황으로 다시 설명해보자.

## 4. Resource와 DB Table

`Resource와 DB Table`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP
흐름에 배치하는 과정이다. 정의 하나를 외우는 것보다 이 개념이 **어느
계층에 있고 어떤 질문에 답하는지**를 이해해야 한다.
...
> **팁**
>
> `Resource와 DB Table`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의
> 실제 상황으로 다시 설명해보자.
...
`API Contract`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에
배치하는 과정이다. 정의 하나를 외우는 것보다 이 개념이 **어느 계층에
있고 어떤 질문에 답하는지**를 이해해야 한다.
...
> **팁**
>
> `API Contract`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제
> 상황으로 다시 설명해보자.

## 7. Orders API

`Orders API`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에
배치하는 과정이다. 정의 하나를 외우는 것보다 이 개념이 **어느 계층에
있고 어떤 질문에 답하는지**를 이해해야 한다.
...
> **팁**
>
> `Orders API`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제
> 상황으로 다시 설명해보자.

## 8. API와 Endpoint

`API와 Endpoint`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에
배치하는 과정이다. 정의 하나를 외우는 것보다 이 개념이 **어느 계층에
있고 어떤 질문에 답하는지**를 이해해야 한다.
...
> **팁**
>
> `API와 Endpoint`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제
> 상황으로 다시 설명해보자.
...
`localStorage API`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에
배치하는 과정이다. 정의 하나를 외우는 것보다 이 개념이 **어느 계층에
있고 어떤 질문에 답하는지**를 이해해야 한다.
...
> **팁**
>
> `localStorage API`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제
> 상황으로 다시 설명해보자.

## 12. REST를 짧게 구분

`REST를 짧게 구분`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에
배치하는 과정이다. 정의 하나를 외우는 것보다 이 개념이 **어느 계층에
있고 어떤 질문에 답하는지**를 이해해야 한다.
...
> **팁**
>
> `REST를 짧게 구분`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제
> 상황으로 다시 설명해보자.
...
1.  `Resource와 API`을 다른 HTTP 구성요소 전체와 같은 개념으로 취급하지
    않는다.
...
1.  **Resource와 API**을 코드 없이 자신의 말로 설명해보자.
...
-   [ ] Resource, API, Endpoint의 관계를 이해하고 API를 단순 URL이나
    Server 자체와 동일시하지 않는다.
...
> **Resource, API, Endpoint의 관계를 이해하고 API를 단순 URL이나 Server
> 자체와 동일시하지 않는다.**

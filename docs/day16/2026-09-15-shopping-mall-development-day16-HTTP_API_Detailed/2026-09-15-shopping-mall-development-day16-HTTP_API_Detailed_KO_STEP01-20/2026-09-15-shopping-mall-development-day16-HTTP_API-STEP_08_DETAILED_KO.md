...
> **이번 STEP의 목표:** POST 주문 생성 흐름, Request Body, Server
> validation, 201 Created, 비멱등성을 이해한다.
...
`createdOrder`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에
배치하는 과정이다. 정의 하나를 외우는 것보다 이 개념이 **어느 계층에
있고 어떤 질문에 답하는지**를 이해해야 한다.
...
> **팁**
>
> `createdOrder`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제
> 상황으로 다시 설명해보자.

## 13. POST와 idempotency
...
1.  `POST와 Create`을 다른 HTTP 구성요소 전체와 같은 개념으로 취급하지
    않는다.
...
1.  **POST와 Create**을 코드 없이 자신의 말로 설명해보자.
...
-   [ ] POST 주문 생성 흐름, Request Body, Server validation, 201
    Created, 비멱등성을 이해한다.
...
> **POST 주문 생성 흐름, Request Body, Server validation, 201 Created,
> 비멱등성을 이해한다.**

# Day 16 --- STEP 15. HTTP와 Promise / async / await

> **이번 STEP의 목표:** fetch의 Promise, await, response.ok, try/catch를
> HTTP 흐름과 연결한다.
...
`pending`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에 배치하는
...
> **팁**
>
> `pending`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제 상황으로
> 다시 설명해보자.
...
`브라우저 전체를 멈추지 않음`는 이번 STEP의 핵심 개념을 주문 프로젝트의
...
> **팁**
>
> `브라우저 전체를 멈추지 않음`를 공부한 뒤 주문 조회·생성·수정·삭제 중
...
`throw`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에 배치하는
과정이다. 정의 하나를 외우는 것보다 이 개념이 **어느 계층에 있고 어떤
질문에 답하는지**를 이해해야 한다.
...
> **팁**
>
> `throw`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제 상황으로
> 다시 설명해보자.
...
1.  `HTTP와 Promise / async / await`을 다른 HTTP 구성요소 전체와 같은
    개념으로 취급하지 않는다.
...
1.  **HTTP와 Promise / async / await**을 코드 없이 자신의 말로
    설명해보자.
...
-   [ ] fetch의 Promise, await, response.ok, try/catch를 HTTP 흐름과
    연결한다.

...
> **이번 STEP의 목표:** Headers와 Body를 구분하고 Content-Type과
> Accept의 역할을 이해한다.
...
`application/json`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에
...
> **팁**
>
> `application/json`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제
> 상황으로 다시 설명해보자.
...
`415 가능성`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에
배치하는 과정이다. 정의 하나를 외우는 것보다 이 개념이 **어느 계층에
있고 어떤 질문에 답하는지**를 이해해야 한다.
...
> **팁**
>
> `415 가능성`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제
> 상황으로 다시 설명해보자.
...
`업무 데이터와 Header`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP
...
> **팁**
>
> `업무 데이터와 Header`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의
> 실제 상황으로 다시 설명해보자.
...
1.  `Headers와 Content-Type`을 다른 HTTP 구성요소 전체와 같은 개념으로
    취급하지 않는다.
...
1.  **Headers와 Content-Type**을 코드 없이 자신의 말로 설명해보자.
...
-   [ ] Headers와 Body를 구분하고 Content-Type과 Accept의 역할을
    이해한다.
...
> **Headers와 Body를 구분하고 Content-Type과 Accept의 역할을 이해한다.**

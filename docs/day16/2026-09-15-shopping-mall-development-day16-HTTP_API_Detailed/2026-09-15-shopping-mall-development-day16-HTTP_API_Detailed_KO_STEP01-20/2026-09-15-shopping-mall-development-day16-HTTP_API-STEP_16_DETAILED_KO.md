# Day 16 --- STEP 16. useOrders CRUD를 HTTP로 번역하기

> **이번 STEP의 목표:** 기존 localStorage CRUD의 의도를 유지하면서
> 데이터 접근 경로를 HTTP API로 바꾼다.
...
`함수 입력/출력`는 이번 STEP의 핵심 개념을 주문 프로젝트의 HTTP 흐름에
...
> **팁**
>
> `함수 입력/출력`를 공부한 뒤 주문 조회·생성·수정·삭제 중 하나의 실제
> 상황으로 다시 설명해보자.

## 9. Server source of truth
...
1.  `useOrders CRUD를 HTTP로 번역하기`을 다른 HTTP 구성요소 전체와 같은
    개념으로 취급하지 않는다.
...
1.  **useOrders CRUD를 HTTP로 번역하기**을 코드 없이 자신의 말로
    설명해보자.
...
-   [ ] 기존 localStorage CRUD의 의도를 유지하면서 데이터 접근 경로를
    HTTP API로 바꾼다.
...
> **기존 localStorage CRUD의 의도를 유지하면서 데이터 접근 경로를 HTTP
> API로 바꾼다.**

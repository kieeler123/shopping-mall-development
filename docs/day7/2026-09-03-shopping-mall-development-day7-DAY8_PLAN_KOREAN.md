...
## 주제 — 주문 취소 기능 (Order Cancellation)

Day 7에서 Dynamic Route, `useParams()`, `Number()`, `find()`, Loading / Not Found / Success로 주문 상세 페이지를 완성했습니다. Day 8에서는 상세 페이지에서 **주문을 취소하고 저장 데이터를 갱신하는 흐름**을 학습합니다.

## 1. 목표
```text
주문 상세 → 취소 버튼 → 사용자 확인 → filter()
→ localStorage 업데이트 → state 업데이트 → /orders 이동
```
> **팁**  
> Day 7은 데이터 읽기, Day 8은 데이터 변경 + 다시 저장하기라고 연결하세요.
...
```
`find()`가 하나를 찾는다면 `filter()`는 조건을 통과한 요소들로 새 배열을 만듭니다.
